...
export default function AutocompleteInput({ 
  ...
  const getFilteredDiagnoses = () => {
    ...
    return DIAGNOSES.filter(d => {
      ...
      const isAttempted = attemptedDiagnoses.some(att => att.id === d.id);
  ...
  const handleSelect = (diagnosis) => {
    ...
    const isHit = onSubmitGuess(diagnosis.id);
    ...
    <div className="w-full max-w-2xl mx-auto flex flex-col items-center">
      ...
      <div ref={wrapperRef} className="relative w-full z-20">
        <input
          ...
          disabled={disabled}
          className="w-full bg-slate-800/80 border border-slate-700 text-slate-100 px-5 py-4 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-400 transition-all placeholder:text-slate-500 disabled:opacity-50 disabled:cursor-not-allowed text-lg shadow-lg"
        />
