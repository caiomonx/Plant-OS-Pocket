...
export default function ChallengeBoard({ caseData, revealedCluesCount, status }) {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto space-y-3 mb-8">
          ...
          return (
             <div 
               ...
               className={`p-5 rounded-xl border transition-all duration-500 ${
                 isRevealed 
                 ? 'bg-slate-800/80 border-cyan-500/30 text-slate-200 shadow-[0_0_15px_rgba(6,182,212,0.05)]' 
                 : 'bg-slate-900/50 border-slate-800 text-slate-700 blur-[2px] select-none opacity-50'
               }`}
             >
                <div className="flex gap-4">
                   <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${
                      isRevealed ? 'bg-cyan-500/20 text-cyan-400' : 'bg-slate-800 text-slate-600'
                   }`}>
                      {index + 1}
                   </div>
       ...
       {status !== 'playing' && caseData.summary && (
          <div className="mt-8 p-6 rounded-xl bg-cyan-900/20 border border-cyan-500/40 text-cyan-50 animate-in fade-in slide-in-from-bottom-4 duration-700">
             <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="font-bold text-cyan-400 uppercase tracking-wide">Sumário Diagnóstico</h3>
             </div>
