...
export default function ImagiologyStatsModal({ isOpen, onClose, stats }) {
  if (!isOpen || !stats) return null;

  const statCards = [
    { label: 'Jogos', value: stats.totalPlayed, accent: 'text-slate-100' },
    { label: 'Win Rate', value: `${stats.winRate}%`, accent: 'text-slate-300' },
    { label: 'Streak Atual', value: stats.currentStreak, accent: 'text-slate-200' },
    { label: 'Maior Streak', value: stats.longestStreak, accent: 'text-slate-400' },
  ];

  // For imagiology, the guess distribution could be "Vidas Restantes"
  // but for now we keep the UI identical and use the mock data provided
  const maxDistribution = Math.max(...(stats.guessDistribution || [0]), 1);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-300">
        ...
        {/* Gradient accent bar (Slate/White theme) */}
        <div className="h-1.5 w-full bg-gradient-to-r from-slate-400 via-slate-300 to-slate-200" />
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center gap-3 bg-slate-800/60 rounded-xl p-3 border border-slate-700/50">
            <div className="flex-1 text-center">
              <span className="text-lg font-bold text-slate-300">{stats.totalWins}</span>
              <p className="text-[10px] text-slate-500 uppercase tracking-widest font-semibold">Vitórias</p>
            </div>
            <div className="w-px h-8 bg-slate-700" />
            <div className="flex-1 text-center">
              <span className="text-lg font-bold text-slate-500">{stats.totalLosses}</span>
          ...
          <div>
            ...
            <div className="space-y-2">
              {stats.guessDistribution?.map((count, index) => {
                ...
                const label = index === 0 ? "3 Vidas" : index === 1 ? "2 Vidas" : "1 Vida";
                    ...
                    <div className="flex-1 relative h-7">
                      <div
                        className={`h-full rounded-md flex items-center justify-end pr-2 transition-all duration-500 ${
                          hasValue
                            ? 'bg-gradient-to-r from-slate-600/80 to-slate-500/90'
                            : 'bg-slate-800'
                        }`}
