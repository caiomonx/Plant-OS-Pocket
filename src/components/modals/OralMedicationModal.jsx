...
export default function OralMedicationModal({ isOpen, onClose, onConfirm, drugConfig }) {
    if (!isOpen || !drugConfig) return null;

    const [selectedOption, setSelectedOption] = React.useState(drugConfig.oralOptions?.[0]);

    const handleConfirm = () => {
        if (!selectedOption) return;
        
        onConfirm({
            doseLabel: selectedOption.label,
            totalContent: selectedOption.dose,
            ...
            <div className="w-full max-w-sm bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-6 relative">
                ...
                <div className="flex items-center gap-3 mb-6 border-b border-slate-800 pb-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-400 mt-1">Administração Via Oral</p>
                    </div>
                </div>
                ...
                <div className="space-y-3 mb-8">
                    ...
                    <div className="grid grid-cols-1 gap-2">
                        {drugConfig.oralOptions?.map((opt, idx) => (
                            <button
                                key={idx}
                                onClick={() => setSelectedOption(opt)}
                                className={`py-3 px-4 rounded-xl text-left text-sm font-bold border transition-colors ${
                                    selectedOption === opt
